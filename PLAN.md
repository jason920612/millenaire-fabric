...
8. ~~**L4 NPC goal 排程器（slice，issue #1）**~~ ✅（2026-06-20）：自訂「單一 `goalKey`、最高 `priority()` 勝、無狀態 singleton goal、per-villager 狀態持久化（`ValueOutput/Input`）」排程器，**非** vanilla 平行 `GoalSelector`；由 `MillWorld` active tick 驅動。4 個 goal（observe_construction/go_to_townhall/wander/idle）。**實測**：建造中選 observe_construction → 建完切 go_to_townhall↔wander 循環（41 次選擇，狀態驅動切換）。**村莊歸屬邊界（採納審計）**：scheduler 改用 **TownHall 的 villager UUID 成員清單**驅動（`getEntity(UUID)`），不再 AABB 掃描；找不到 entity 會記錄（repair/respawn 為後續）。**chunk forcing（#6 部分）**：active force-load + **啟動時釋放殘留 forced chunks**（runtime-only `active` 的洩漏修正，實測 `released leftover forced chunks`）。**villager repair（issue #11）✅**：成員記錄擴成 `VillagerMember`（UUID+name+home）；active 村莊發現 UUID missing 且 home chunk 已載入時，用**同 UUID 重生**村民。**duplicate-UUID guard（P1 修補）**：repair 前等 `REPAIR_GRACE_TICKS=60`（讓存檔實體先載入，避免 race 誤判）；`spawn()` 回 `Optional`，`addFreshEntity` 被拒不當成功、scheduler 不 tick 未入世界物件。實測：save/kill/reload 後 0 個 duplicate-UUID 警告。**data-driven goals（issue #1，slice）✅**：解析 villager type 的 `goal=` 清單（`VillagerType`）；scheduler 候選 goal = **該 villager type 的內容 goal**（`GenericGoal`/`GenericGoalDefinition`）+ 4 個硬編 fallback；villager 帶 type（持久化、repair 保留）。**實測**：abbot 選 chat/gopray/gosocialise、alchemist 選 brewpotions/makeglassbottles…（內容定義的 key，非硬編）；reload+repair 後仍依 type 選 goal。**goal 定義解析（issue #4，slice）✅**：解析 `millenaire/goals/**`（135 個定義）→ generic goal 取得**真實 priority/duration（ms→tick）**（`GoalDefinitions`）；無定義的硬編 key（gopray/chat…）退回 placeholder。goal key 大小寫正規化、`Culture.villagerType` key-insensitive。**實測**：`makeglassbottles` priority=50 duration=20t。
   - **destination 欄位修正（採納審計 P1）**：原本誤讀不存在的 `tag`；改讀 **`buildingTag`**（目的地建築）+ **`requiredTag`**（升級/解鎖篩選）+ `townhallgoal`。`BuildingProject` 持久化建築 tags；`TownHall.findBuildingByTag(buildingTag, requiredTag)` 解析目的地；`GenericGoal.isPossible` 用它做 building-存在/解鎖 gating。**實測 aggregate**：`135 (30 buildingTag dest, 2 townhall, 41 requiredTag)`（對上 content 統計）。
   - **requiredTag-only gating（採納審計 P1）**：即使 destination 是 house，只要 goal 有 `requiredTag` 就要求村莊存在該（解鎖）建築。實測（真實 `isPossible`）：`makejgboots(requiredTag=armoury)=false`、`makecalva(buildingTag=cider)=false`、`cookindianbrick(townhall)=true`，全部符合預期。
   - **未達**：真實 goal **行為**（craft in/out、soil/harvest、抵達後生產，接 #3）、house destination、building-dest **端到端行走**（需先生成 core 工作建築如 bakery/forge 並有對應職業村民）、其餘內容 DSL（shops/namelists/languages/quests/traded_goods）。
9. **剩餘工作（全功能對等的長路）**：
